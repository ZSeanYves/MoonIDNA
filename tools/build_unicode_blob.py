...
def write_outputs(blob: bytes, manifest: dict[str, object], binary: Path, embed: Path, lock: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="moonidna-embed-") as temporary:
        temporary_path = Path(temporary)
        chunks = [blob[offset : offset + EMBED_CHUNK_SIZE] for offset in range(0, len(blob), EMBED_CHUNK_SIZE)]
        for index, chunk in enumerate(chunks):
            chunk_binary = temporary_path / f"unicode_data_{index}.bin"
            chunk_embed = temporary_path / f"unicode_data_{index}.mbt"
            ...
            subprocess.run(
                [
                    ...
                    str(chunk_embed),
                    "--name",
                    f"unicode_data_blob_{index}",
                ],
                cwd=ROOT,
                check=True,
            )
            generated = chunk_embed.read_text(encoding="utf-8")
    ...
    for index in range(len(chunks) - 1):
        ...
        accessor.append(f"    return unicode_data_blob_{index}[offset - {index * EMBED_CHUNK_SIZE}]\n")
        accessor.append("  }\n")
    last = len(chunks) - 1
    accessor.append(f"  unicode_data_blob_{last}[offset - {last * EMBED_CHUNK_SIZE}]\n")
    accessor.append("}\n")
    embed.write_text("\n".join(generated_parts) + "".join(accessor), encoding="utf-8")
